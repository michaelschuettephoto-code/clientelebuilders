...
-- RLS policy: Only admins can manage roles (we'll use security definer function)
...
-- Insert existing roles from profiles into user_roles
...
-- Create restrictive policy: Users can only view own profile
...
-- Update policy: Users can update only own profile (not other fields would be restricted in app logic)
